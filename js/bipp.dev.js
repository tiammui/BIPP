...
var bipp = {
  ...
  compression: true,
  autoSelect: false,
  startBipp: function startBipp(config) {
    ...
    this["var"].canvasContext = this["var"].canvas.getContext("2d"); // START EVENT TRIGGERS

    bipp.utility.eventEmit("start");
  },
  decodeClass: function decodeClass(pictureUrl, bippStyle, classPort) {
    if (!pictureUrl) {
    ...
    if (classPort == this["var"].currentPort) {} else {
      ...
      if (this["var"].classPorts.length == 0) {
        ...
        for (i = 0; i < this["var"].classPorts.length; i++) {
  ...
  decodeID: function decodeID(pictureUrl, bippStyle, IDPort) {
    var IDdecodes = this["var"].IDdecodes;
    var decodesPlaceholder = [];

    for (i = 0; i < IDdecodes.length; i++) {
    ...
    if (port.className.indexOf("bipp") == -1) {
      for (i = 0; i < styleListValue.length; i++) {
      ...
      var bippCorrect = styleListValue.some(function (x) {
        return x == styleValue;
      });
      if (bippCorrect) port.className = port.className.replace(styleListValue[0], styleValue).replace(styleListValue[1], styleValue).replace(styleListValue[2], styleValue).replace(styleListValue[3], styleValue).replace(styleListValue[4], styleValue);else console.error("Invalid BIPPStyle cannot be passed to " + port + "(ID of an HTMLElement)");
    }
  },
  bippElement: function bippElement(pic, style, port) {
    this.pictureUrl = pic;
    this.bippStyle = style;
    this.port = port;
  },
  refreshAllClass: function refreshAllClass() {
    ...
    for (x = 0; x < ports.length; x++) {
      ...
      for (i = 0; i < this["var"].classDecodes.length; i++) {
      ...
      for (i = 0; i < queryResult.length; i++) {
        ...
        if (currentPort[i].className.indexOf("bipp") == -1) {
          for (ii = 0; ii < styleListValue.length; ii++) {
  ...
  refreshAllID: function refreshAllID() {
  ...
  refreshClass: function refreshClass(classPort) {
    var ports = this["var"].classPorts;
    ...
    if (index !== -1) {
      ...
      console.error(new Error("The classPort to be refreshed have not been decoded"));
    }
  },
  refreshID: function refreshID(IDport) {
    var ports = this["var"].IDdecodes;
    var index = -1;
  ...
  refreshAll: function refreshAll() {
    ...
    this.refreshAllClass(); // To Re-render IDPorts

    this.refreshAllID();
  },
  utility: {
    frameProcessor: function frameProcessor(width, height, transferredUrl, low, high) {
      bipp["var"].imgRatio = width / height;

      if (bipp["var"].imgRatio < low && !bipp.autoSelect) {
        bipp["var"].framesCon.innerHTML = "";

        for (i = 0; i < bipp["var"].heightList.length; i++) {
        ...
        for (i = 0; i < bipp["var"].widthList.length; i++) {
        ...
        bipp["var"].style = bipp["var"].styleRadio[0].value; // frameSelect EVENT TRIGGER - before image compression

        bipp.utility.eventEmit("frameSelect");
        this.compressImage();
      }

      for (i = 0; i < bipp["var"].frame.length; i++) {
        bipp["var"].frame[i].style.backgroundImage = "url(" + transferredUrl + ")";
      }
    },
    inputChange: function inputChange(e) {
      // Listen to image input
      //Get file
      var bippFile = bipp["var"].bippFile = e.target.files[0];
      ...
      if (bippFile && bippFile.type.indexOf("image") != -1) {
        ...
        bipp["var"].uploaderCon.style.display = "block"; // To prevent throwing of Uncaught error if bippFile == undefined
    ...
    imageLoad: function imageLoad(e) {
      ...
      bipp["var"].image.style.display = "none"; //Aspect ratio ranges
      ...
      var AB = 1.17,
          ...
          HI = 0.71,
          AI = 0.88;
      var Arange = ratio >= AI && ratio < AB,
          Brange = ratio >= AB && ratio < BC,
          Crange = ratio >= BC && ratio < CD,
          Drange = ratio >= CD && ratio < DE,
          Erange = ratio >= DE,
          Frange = ratio < FG,
          Grange = ratio >= FG && ratio < GH,
          Hrange = ratio >= GH && ratio < HI,
          Irange = ratio >= HI && ratio < AI;
      ...
      bipp.utility.frameProcessor(bipp["var"].image.clientWidth, bipp["var"].image.clientHeight, bipp["var"].image.src, bipp["var"].low, bipp["var"].high);
    },
    checkStyle: function checkStyle(position) {
      for (i = 0; i < bipp["var"].curtain.length; i++) {
        bipp["var"].curtain[i].style.display = "none";
      }

      bipp["var"].style = bipp["var"].styleRadio[position].value;
      bipp["var"].curtain[position].style.display = "block"; // frameSelect EVENT TRIGGER - before image compression
    ...
    compressImage: function compressImage() {
      ...
      if (blobCanBeCreated) {
        ...
        var BIPP_CANVAS_WIDTH = bipp.width;
        var BIPP_CANVAS_RATIO = bipp.aspectRatio;
        var BIPP_CANVAS_HEIGHT = bipp.width / BIPP_CANVAS_RATIO;
        var bippStyleStyle = bipp.getStyle().split("-")[0];
        var imgRatio = bipp["var"].imgRatio; // use the bippstyle to draw image on this.var.Canvas
        ...
        switch (bippStyleStyle) {
          ...
          default:
            console.error("bippstyle is empty");
            break;
        }

        bipp["var"].canvas.toBlob(function (blob) {
          // Gave the blob a "name" and "lastModifiedDate" to make it a file
          blob.name = bipp["var"].bippFile.name.split(".")[0] + ".jpg";
          blob.lastModifiedDate = new Date();
          bipp["var"].bippFile = blob;
        }, "image/jpeg"); // afterCompress EVENT TRIGGER

        bipp.utility.eventEmit("afterCompress", "Compression was successful");
      } else {
        if (bipp.compression) {
          console.log("%c Compression is set to false in BIPP config, provided image won't be compressed.\n 'beforeCompress' and 'afterCompress' events won't trigger", "color:blue"); // afterCompress EVENT TRIGGER

          bipp.utility.eventEmit("afterCompress", "compression is false");
        } else {
          console.error("Browser doesn't support canvas rendering or HTMLElement.toBlob() method, compressed version of inputed image can't be created. \n The uncompressed original image inputed by the user is passed to 'bipp.getFile()' instead."); // afterCompress EVENT TRIGGER
    ...
    eventEmit: function eventEmit(eventName, eventLoad) {
      ...
      switch (eventName) {
        ...
        default:
          console.error("no such eventName ---->" + eventName);
      }
    },
    hover: function hover(e, eventType) {
      switch (eventType) {
        case "frame":
  ...
  selectImage: function selectImage() {
    ...
    for (i = 0; i < eventReturns.length; i++) {
  ...
  events: {
    ...
    //
    beforeSelect: [],
    //if any callback return boolean false, image selection won't be possible
    afterSelect: [],
    //(original image-if present,selection status)
    beforeCompress: [],
    //(original image)
    afterCompress: [],
    //(compressed image,compression status) similar to 'frameSelect' event
    frameSelect: [],
    //(original/compressed image,BIPPstyle) depending on compression state
    frameHover: [],
    // (hover event)
    uploaderHover: [],
    // (hover event)
